...
foreach ($ship_sub_badges as $subclass) {
  ...
  if (strpos($enabled, $subclass['name'])) {
    $subclass['cnt'] = $cnt;
...
{
	$group = $row['itm_grp'];
	if(in_array($group,array(89,90,384,385,386,387,394,395,396,476,507,508,509,510,511,524,653,654,655,656,657,771,772,862,863,864)))
		$group = 506;
	if (in_array($group, array(544, 549, 639, 641)))
		$group = 100;
	switch ($group)
	{
