...
if(isset($_GET['page'])) $pagelim = intval($_GET['page']);
else $pagelim = false;
...
switch ($_GET['view'])
{
	...
	case "pilot_ranks":
		...
		{ $limit = 0; }
		$query = "SELECT plt_id FROM `kb3_pilots` INNER JOIN `kb3_corps` on kb3_pilots.plt_crp_id = kb3_corps.crp_id
            WHERE crp_all_id =".$alliance->getID()."
	...
	case "pilot_medals":
		$qry = new DBQuery();
		$query = "SELECT rank.med_id AS med_id, rank.time_id AS time_id, rank.plt_id AS plt_id FROM `kb3_rank_medals` rank INNER JOIN `kb3_pilots` plts ON ( plts.plt_id = rank.plt_id ) INNER JOIN `kb3_corps` corps on plts.plt_crp_id = corps.crp_id WHERE corps.crp_all_id = ".$alliance->getID()." ORDER BY 1 ASC, 2 DESC";
		$qry->execute($query);
		$meds = array();
		while ($row = $qry->getRow())
		{
		...
		foreach($medsconv as $month=>$awards)
		{
			...
			for ($t = 0; $t <= 8; $t++)
			{
				...
				if($awards[$t])
				{ $html .= "<td><a class=kb-shipclass href=?a=pilot_detail&amp;plt_id=".$pilot->getID()."><img src=\"".$pilot->getPortraitURL()."\" /></a></td>"; } else
				{ $html .= "<td>&nbsp;</td>"; }
			}
			$html .= "</tr>";
		}
	...
	case "rank_ribbons":
		...
		if ($keep_title)
		{ $words = 'Custom Rank Set'; } else
		{ $words = $rank_titleset." Rank Set"; }
	...
	case "known_members":
		...
		$query = "SELECT plt_id FROM kb3_pilots plt
