...
function update008()
{
	...
	if (CURRENT_DB_UPDATE < "008" )
	{
		...
		$qry->execute("INSERT INTO kb3_config (SELECT distinct cfg_site,
			'DBUpdate', '008' FROM `kb3_config`)
			ON DUPLICATE KEY UPDATE cfg_value = '008'");
...
function update009()
{
	...
	if (CURRENT_DB_UPDATE < "009" )
	{
		...
		$qry->execute("INSERT INTO kb3_config (SELECT distinct cfg_site,
			'DBUpdate', '009' FROM `kb3_config`)
			ON DUPLICATE KEY UPDATE cfg_value = '009'");
		echo $header;
...
function update010()
{
	...
	if (CURRENT_DB_UPDATE < "010" )
	{
		...
		$qry->execute("INSERT INTO kb3_config (SELECT distinct cfg_site,
			'DBUpdate', '010' FROM `kb3_config`)
			ON DUPLICATE KEY UPDATE cfg_value = '010'");
		echo $header;
...
function update011()
{
	...
	if (CURRENT_DB_UPDATE < "011" )
	{
		...
		$qry->execute("INSERT INTO kb3_config (SELECT distinct cfg_site,
			'DBUpdate', '011' FROM `kb3_config`)
			ON DUPLICATE KEY UPDATE cfg_value = '011'");
		echo $header;
